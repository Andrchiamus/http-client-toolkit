...
| `respectCacheHeaders` | `boolean` | `false` | Respect HTTP cache headers (RFC 9111) |
| `cacheHeaderOverrides` | `CacheHeaderOverrides` | — | Override specific cache header behaviors |
...
When `respectCacheHeaders: true`, the client respects `Cache-Control`, `ETag`, `Last-Modified`, and `Expires` headers per RFC 9111. See the [Cache Headers guide](/http-client-toolkit/guides/cache-headers/) for details.
